...
(
    ...
    type        VARCHAR(30) NOT NULL CHECK (type IN
                                            ('SPACE_TYPE', 'INSTRUMENT_EQUIPMENT', 'AMENITY', 'OTHER_FEATURE')),
...
-- =============================================
COMMENT
ON TABLE place_info IS '업체 기본 정보 테이블';
COMMENT
...
COMMENT
ON TABLE place_locations IS '업체 위치 정보 테이블';
COMMENT
ON COLUMN place_locations.coordinates IS 'PostGIS 지리 좌표 (Point)';
COMMENT
ON COLUMN place_locations.location_guide IS '위치 안내 (예: 2호선 강남역 3번 출구 도보 5분)';

COMMENT
ON TABLE place_contacts IS '업체 연락처 정보 테이블';
COMMENT
ON TABLE place_websites IS '업체 웹사이트 URL 테이블';
COMMENT
ON TABLE place_social_links IS '업체 소셜 미디어 링크 테이블';

COMMENT
ON TABLE place_parkings IS '업체 주차 정보 테이블';
COMMENT
ON COLUMN place_parkings.parking_type IS '주차 타입 (FREE: 무료, PAID: 유료)';

COMMENT
ON TABLE place_images IS '업체 이미지 테이블';
COMMENT
