...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'keywords') THEN
        ...
        (
            ...
            type        VARCHAR(30) NOT NULL CHECK (type IN ('SPACE_TYPE', 'INSTRUMENT_EQUIPMENT', 'AMENITY',
                                                             'OTHER_FEATURE')),
...
FROM (VALUES
          ...
          ('깔끔한 인테리어', 'OTHER_FEATURE', '깔끔한 인테리어', 36, true)) AS v(name, type, description, display_order, is_active)
